...
RUN ls -l /etc/nginx
#link nginx file
RUN if [ "${ENV}" = "prod" ]; then cp -f /data/workspace/democart/democart.conf /etc/nginx/vhosts.d/democart.conf; else ln -s /data/workspace/democart/democart-${ENV}.conf /etc/nginx/vhosts.d/democart.conf; fi
